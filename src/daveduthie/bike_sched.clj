...
(s/def :mode/req (s/coll-of (s/tuple :r/quantity uuid?)
                            :distinct true
                            :gen-max 4))
...
(s/def ::schedule (s/keys :req [:r/tasks
                                :r/resources
                                :r/modes]))

(defn make-task-refs-consistent-impl [tasks dep-ref-replacements mode-ref-replacements]
  (into {} (map (fn [[task-id task-val]]
                  [task-id
                   (-> task-val
                       (update :deps/ids
                               #(->> %
...
(defn make-task-refs-consistent [{:r/keys [tasks modes] :as schedule}]
  (let [dep-ref-replacements  (zipmap (sort (mapcat :deps/ids (vals tasks)))
        ...
        mode-ref-replacements (zipmap (sort (mapcat :modes/ids (vals tasks)))
                                      (sort (keys modes)))]
    (update schedule :r/tasks
            make-task-refs-consistent-impl dep-ref-replacements mode-ref-replacements)))

(defn make-mode-refs-consistent-impl [modes resource-ref-replacements]
  (into {}
...
(defn make-mode-refs-consistent [{:r/keys [modes resources] :as schedule}]
  (let [available-resource-refs   (sort (keys resources))
    ...
    (update schedule :r/modes
            make-mode-refs-consistent-impl resource-ref-replacements)))

(defn prune-mode-refs [{:r/keys [modes] :as schedule}]
  (update schedule :r/modes
          (partial into {} (filter (comp not-empty :mode/req val)))))

(defn consistent-schedule [{:r/keys [tasks modes] :as schedule}]
  (-> schedule
      make-mode-refs-consistent
      prune-mode-refs
      make-task-refs-consistent))

(defn summarise [{:r/keys [tasks modes] :as schedule}]
...
(defn sched [size seed]
  (gen/generate
   (gen/fmap
    consistent-schedule
    (->> (s/gen ::schedule) (gen/such-that (comp not-empty :r/tasks))
         (gen/such-that (comp not-empty :r/resources))
         (gen/such-that (comp not-empty :r/modes)))) size seed))

(defn -main [& args]
  (let [parsed (map #(Integer/parseInt %) args)]
    (println (json/write-value-as-string
              (summarise (apply sched parsed))))))
